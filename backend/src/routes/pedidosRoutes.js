...
router.get('/:pedidoId', (req, res) => {
  ...
  connection.query(queryPedido, [pedidoId], (err, pedidoResults) => {
    if (err) {
    ...
    connection.query(queryItens, [pedidoId], (err, itensResults) => {
      if (err) {
...
router.delete('/:pedidoId', (req, res) => {
  ...
  const query = "UPDATE pedidos SET status = 'Cancelado' WHERE id = ? AND status = 'Aguardando Pagamento'";
